...
def delete_records(jsondat, last_record = 0):
    batch_size = 200
    ...
    if last_record < total_records:
        delete_records(jsondat, last_record)

def delete_request(payload):
    response = requests.delete(url, data=json.dumps(payload), headers=headers, verify=False)
    code = response.status_code
    print("Delete Response code: ", code)
    if code == 202:
